...
<script>
import UserEdit from "../publication/components/publicationEdit";
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          key: 'PDF',
          render: (h, params) => {
            let path = params.row.PDF.substr(31)
            return h('span',path)
          }
        },
        {
  ...
  methods: {
    ...
    queryUserByPagination () {
      this.$http.get('publication/queryUserByPagination',{
        params: {
          page: this.page,
          rows: this.rows,
        }
    ...
    deleteConfirm (userId) {
      this.$Modal.remove()
      this.$http.post('publication/deleteUserById', {
        id: userId
      },{
        _this: this,
        loading: 'loading'
